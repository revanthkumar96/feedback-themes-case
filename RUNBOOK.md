...
three-tier hierarchy cannot be contradicted. Subject-free verdicts and known
out-of-scope subjects route to the explicit `Other feedback` branch (general
sentiment / out-of-scope leaves), so every review lands somewhere in the
three-tier tree. An empty assignment list remains legal (surfaced as
`no_relevant_theme` in the rich output) and means the review discusses a
concrete subject that even the extended taxonomy does not cover.
...
roughly 120k. Comparison runs and failed attempts count against the same
...
sections from the checker (317 rows, 5 / 16 / 38 hierarchy, all 223 reviews
covered); an evaluation report written to `out/evaluation.json`. Reference
numbers for the submitted run: micro-F1 0.769, precision 0.764, recall 0.775,
exact match 0.660, evidence validity 1.000.
...
2. A human replaces each `null` with the list of supported leaf IDs, judging
   only against the frozen theme definitions. Since taxonomy v1.1,
   endorsements, pure sentiment ("Never again."), and emoji-only reviews are
   annotated with the matching general-sentiment leaf, and known out-of-scope
   subjects with the matching out-of-scope leaf. The empty list `[]` remains
   a deliberate annotation for concrete subjects no leaf covers (currently
   two reviews: remote e-signing and delegated accountant access).
...
- **Unattributed cost complaints never guess.** "Too expensive" without an
identified fee or interest mechanism lands in the unattributed-price-remarks
leaf, not in a fee or interest leaf.
- **Sentiment leaves are a last resort.** A general-sentiment leaf applies
only when the whole review names no concrete subject, and never alongside a
subject assignment.
...
- **Remaining coverage gaps** (model files under the nearest leaf instead of
abstaining): user and role management, remote e-signing. Taxonomy v1.1
already closed credit enquiries, account administration, marketing
communications, and unattributed price remarks.
...
- **Sentiment leaves can over-absorb**: a review that does name a subject
("Finally a bank where I don't have to meet anyone in person to sign") is
